...
export default {
  ...
  methods: {
    moment,
    goPage (parameter) {
      return getInfoList(Object.assign(parameter,this.queryParam)).then(res => {
        return res.result
      })
    },
    // 搜索展开关闭
    ...
    // 新增
    add () {
      this.$router.push({ name: 'InformationEdit' , query: { state: 'add' }})
    },
    // 编辑
    handleEdit (text, record, index) {
      this.$router.push({ name: 'InformationEdit' , query: { state: 'edit', id : record.id } })
      console.log(text)
    ...
    // 单个删除
    handleDel(record){
      getInfoList(record.id).then(res => {
        this.$refs.table.refresh();  //重新加载数据
      })
    },
