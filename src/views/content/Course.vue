        ...
        <a-card :bordered="false">
          <a-form :form="form">
            ...
            <a-form-item label="课程"
              ...
              <a-select  placeholder="请选择课程" default-value="0" :disabled="lookdis"
                         v-decorator="['kc',{rules: [{ required: true, message: '请选择课程' }]}]">
                <a-select-option :key="c.id" v-for="c in option">{{ c.value }}</a-select-option>
              </a-select>
            </a-form-item>
    ...
    >
      ...
      <span slot="action" slot-scope="text, record, index">
        <template>
          ...
          <a-divider type="vertical"/>
          <a title="推广码" @click="handleExt(text, record, index)"><a-icon type="notification" /></a>
          <a-divider type="vertical"/>
...
<script>
  ...
  import { getCouList,getCouDet } from '@/api/course'
  ...
  export default {
    ...
    methods: {
      ...
      // 列表查询
      goPage (parameter) {
        return getCouList(Object.assign(parameter,this.queryParam)).then(res => {
          return res.result
        })
      },
      // 统一删除
