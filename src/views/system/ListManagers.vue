...
import { getListManagersList,getListManagersDet } from '@/api/system'
...
export default {
  ...
  methods: {
    moment,
    goPage (parameter) {
      return getListManagersList(Object.assign(parameter, this.queryParam)).then(res => {
        return res.result
      })
    },
    // 搜索展开关闭
    ...
    del () {
      ...
      console.log(params)
      getListManagersDet(params).then(res => {
    ...
    // 新增
    add () {
      this.$router.push({ name: 'ListManagersEdit', query: { state: 'add' } })
    },
    // 编辑
    handleEdit (text, record, index) {
      this.$router.push({ name: 'ListManagersEdit', query: { state: 'edit', id: record.id } })
      console.log(text)
    ...
    // 单个删除
    handleDel (record) {
      getListManagersDet(record.id).then(res => {
        this.$refs.table.refresh() // 重新加载数据
      })
    },
